      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="px-4 py-4">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowAmount(!showAmount)} className="text-gray-500">
                {showAmount ? '👁' : '👁‍🗨'}
              </button>
            </div>
          ...
          <div className="grid grid-cols-3 gap-4 mt-4 text-sm">
            ...
            <div className="text-right">
              <div className="text-gray-500 mb-1">现金 &gt;</div>
        ...
        <div className="px-4 pt-4">
          <div className="flex items-center gap-6 border-b border-gray-800 pb-3">
            <button
              ...
              className={`font-medium ${activeTab === 'positions' ? 'text-white' : 'text-gray-500'}`}
            >
              持仓分布
            </button>
            <button
              onClick={() => setActiveTab('orders')}
              className={`text-sm ${activeTab === 'orders' ? 'text-white' : 'text-gray-500'}`}
            >
              当日订单 (0/0)
            </button>
            <button
              onClick={() => setActiveTab('strategy')}
              className={`text-sm ${activeTab === 'strategy' ? 'text-white' : 'text-gray-500'}`}
            >
              策略订单
            </button>
        ...
        <div className="px-4 py-3">
          ...
          >
            ...
            <span className="text-gray-500">{expandedMarket === 'US' ? '∧' : '∨'}</span>
          </button>

          {expandedMarket === 'US' && (
            <>
              {/* 美股汇总 */}
              <div className="grid grid-cols-3 gap-4 py-3 text-sm border-b border-gray-800/50">
                <div>
                  <div className="text-gray-500 text-xs mb-1">持仓市值</div>
                  <div>{usTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
                </div>
                <div>
                  <div className="text-gray-500 text-xs mb-1">持仓盈亏</div>
                  <div className={usProfitLoss >= 0 ? 'text-green-500' : 'text-red-500'}>
                    {usProfitLoss >= 0 ? '+' : ''}{usProfitLoss.toFixed(2)}
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-gray-500 text-xs mb-1">当日盈亏</div>
              ...
              {/* 表头 */}
              <div className="grid grid-cols-4 gap-2 py-2 text-xs text-gray-500">
                <div>名称/代码</div>
              ...
              {usPositions.map(position => {
                const todayPLPercent = ((position.currentPrice - position.costPrice) / position.costPrice * 100);
                return (
                  <Link
                    key={position.symbol}
                    href={`/stock/${position.market}/${position.symbol}`}
                    className="grid grid-cols-4 gap-2 py-3 border-t border-gray-800/30 hover:bg-gray-800/30 transition"
                  >
                    <div>
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-gray-500">{position.quantity}</div>
                    </div>
                    <div className="text-right">
                      <div>{position.currentPrice.toFixed(3)}</div>
                      <div className="text-xs text-gray-500">{position.costPrice.toFixed(3)}</div>
                    </div>
                    <div className="text-right">
        ...
        <div className="px-4 py-3 border-t border-gray-800">
          ...
          >
            ...
            <span className="text-gray-500">{expandedMarket === 'HK' ? '∧' : '∨'}</span>
          </button>
        ...
        <div className="flex justify-center py-6">
          <button className="flex items-center gap-2 px-6 py-2 border border-gray-700 rounded-full text-gray-400 hover:bg-gray-800 transition">
            <span>✏️</span>
            <span>编辑</span>
          </button>
        </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-gray-900 border-t border-gray-800">
        <div className="max-w-lg mx-auto flex">
          {[
            ...
            { icon: '💰', label: '资产', href: '/positions', active: true },
